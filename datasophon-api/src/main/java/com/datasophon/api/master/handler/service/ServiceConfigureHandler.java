...
public class ServiceConfigureHandler extends ServiceHandler {
    ...
    public ExecResult handlerRequest(ServiceRoleInfo serviceRoleInfo) throws Exception {
        ...
        generateServiceConfigCommand.setDecompressPackageName(
                serviceRoleInfo.getDecompressPackageName());
        generateServiceConfigCommand.setRunAs(serviceRoleInfo.getRunAs());
        if ("zkserver".equals(serviceRoleInfo.getName().toLowerCase())) {
            generateServiceConfigCommand.setMyid(
        ...
        ActorSelection configActor =
                ActorUtils.actorSystem.actorSelection(
                        "akka.tcp://datasophon@"
                                ...
                                + ":2552/user/worker/configureServiceActor");

        Timeout timeout = new Timeout(Duration.create(180, TimeUnit.SECONDS));
        Future<Object> configureFuture =
                Patterns.ask(configActor, generateServiceConfigCommand, timeout);
        try {
