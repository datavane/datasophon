...
public class ServiceInstallHandler extends ServiceHandler {
    ...
    public ExecResult handlerRequest(ServiceRoleInfo serviceRoleInfo) throws Exception {
        ...
        ClusterHostEntity hostEntity =
                clusterHostService.getClusterHostByHostname(serviceRoleInfo.getHostname());
        ...
        installServiceRoleCommand.setDecompressPackageName(
        ...
        String md5 =
                FileUtil.readString(
                        Constants.MASTER_MANAGE_PACKAGE_PATH
                                + Constants.SLASH
                                + serviceRoleInfo.getPackageName()
                                + ".md5",
        ...
        if ("aarch64".equals(hostEntity.getCpuArchitecture())
                && FileUtil.exist(
                        Constants.MASTER_MANAGE_PACKAGE_PATH
                                ...
                                + serviceRoleInfo.getDecompressPackageName()
                                + "-arm.tar.gz")) {
            installServiceRoleCommand.setPackageName(
        ...
        ActorSelection actorSelection =
                ActorUtils.actorSystem.actorSelection(
                        "akka.tcp://datasophon@"
                                + serviceRoleInfo.getHostname()
                                + ":2552/user/worker/installServiceActor");
