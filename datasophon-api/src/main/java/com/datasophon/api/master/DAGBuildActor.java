    ...
    public void onReceive(Object message) throws Throwable {
        if (message instanceof StartExecuteCommandCommand) {
            ...
            ClusterInfoEntity clusterInfo =
                    clusterInfoService.getById(executeCommandCommand.getClusterId());
            List<ClusterServiceCommandEntity> commandList =
                    commandService
                            .lambdaQuery()
                            .in(
                                    ClusterServiceCommandEntity::getCommandId,
                                    executeCommandCommand.getCommandIds())
                            .list();
            ...
            if (ArrayUtil.isNotEmpty(commandList)) {
                for (ClusterServiceCommandEntity command : commandList) {
                    ...
                    List<ClusterServiceCommandHostCommandEntity> hostCommandList =
                            hostCommandService.getHostCommandListByCommandId(
                    ...
                    for (ClusterServiceCommandHostCommandEntity hostCommand : hostCommandList) {
                        ...
                        ServiceRoleInfo serviceRoleInfo =
                                JSONObject.parseObject(
                                        frameServiceRoleEntity.getServiceRoleJson(),
                                        ServiceRoleInfo.class);
                        ...
                        serviceRoleInfo.setDecompressPackageName(
                                serviceEntity.getDecompressPackageName());
                        ...
                        if (Objects.nonNull(serviceRoleHandler)) {
                            serviceRoleHandler.handlerServiceRoleInfo(
                                    serviceRoleInfo, hostCommand.getHostname());
                        }
                ...
                for (FrameServiceEntity serviceEntity : frameServiceList) {
                    if (StringUtils.isNotBlank(serviceEntity.getDependencies())) {
                        for (String dependency :
                                serviceEntity.getDependencies().split(Constants.COMMA)) {
            ...
            SubmitActiveTaskNodeCommand submitActiveTaskNodeCommand =
                    new SubmitActiveTaskNodeCommand();
