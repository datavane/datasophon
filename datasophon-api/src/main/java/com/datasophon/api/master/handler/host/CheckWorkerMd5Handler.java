...
public class CheckWorkerMd5Handler implements DispatcherWorkerHandler {
    ...
    public boolean handle(ClientSession session, HostInfo hostInfo) {
        String checkWorkerMd5Result =
                MinaUtils.execCmdWithResult(session, Constants.CHECK_WORKER_MD5_CMD).trim();
        String md5 =
                FileUtil.readString(
        ...
        hostInfo.setMessage(
                MessageResolverUtils.getMessage(
                        "md5.verification.successful.and.installation.package.decompressed"));
        return true;
    }
}
