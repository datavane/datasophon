...
public class UserPermissionHandler implements HandlerInterceptor {
    ...
    public boolean preHandle(
        ...
        if (handler instanceof HandlerMethod) {
            ...
            UserPermission annotation =
                    handlerMethod.getMethod().getAnnotation(UserPermission.class);
            if (Objects.nonNull(annotation)) {
                UserInfoEntity authUser =
                        (UserInfoEntity) request.getSession().getAttribute(Constants.SESSION_USER);
                ...
                if (Objects.nonNull(authUser)) {
                    if (!SecurityUtils.isAdmin(authUser)) {
                        ...
                        if (parameterMap.containsKey("clusterId")) {
                            ...
                            if (clusterUserService.isClusterManager(
                                    authUser.getId(), clusterIds[0])) {
