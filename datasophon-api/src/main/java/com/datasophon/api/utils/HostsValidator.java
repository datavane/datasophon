...
public class HostsValidator implements ConstraintValidator<Hosts, String> {
    ...
    public boolean isValid(String value, ConstraintValidatorContext context) {
        ...
        // if (ALPHABET.matcher(value).matches() && (value.contains(Constants.CENTER_BRACKET_LEFT)
        // || value.contains(Constants.CENTER_BRACKET_RIGHT) || value.contains(Constants.HYPHEN))) {
        if (flagOne && flagFive) {
            return false;
        }
        // 4. [ - ] 三个字符顺序不能颠倒
        ...
        for (String item : items) {
            ...
            if (IP.matcher(item).matches()) {
                ...
                for (int i = 0; i < splitLen; i++) {
                    ...
                    // if ((i != 0 && ipItems[i].startsWith("0")) || (curLen > 1 &&
                    // ipItems[i].startsWith("0"))) {
