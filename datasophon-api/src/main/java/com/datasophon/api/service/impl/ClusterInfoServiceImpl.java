...
public class ClusterInfoServiceImpl extends ServiceImpl<ClusterInfoMapper, ClusterInfoEntity>
    ...
    @Autowired private ClusterInfoMapper clusterInfoMapper;

    @Autowired private ClusterRoleUserService clusterUserService;

    @Autowired private AlertGroupService alertGroupService;

    @Autowired private ClusterAlertGroupMapService groupMapService;
    ...
    @Autowired private ClusterYarnSchedulerService yarnSchedulerService;

    @Autowired private ClusterNodeLabelService nodeLabelService;

    @Autowired private ClusterQueueCapacityService queueCapacityService;

    @Autowired private ClusterRackService rackService;

    @Override
    public ClusterInfoEntity getClusterByClusterCode(String clusterCode) {
        ClusterInfoEntity clusterInfoEntity =
                clusterInfoMapper.getClusterByClusterCode(clusterCode);
    ...
    public Result saveCluster(ClusterInfoEntity clusterInfo) {
        List<ClusterInfoEntity> list =
                this.lambdaQuery()
                        .eq(ClusterInfoEntity::getClusterCode, clusterInfo.getClusterCode())
                        .list();
    ...
    private void putClusterVariable(ClusterInfoEntity clusterInfo) {
        ...
        for (FrameServiceEntity frameServiceEntity : frameServiceList) {
            globalVariables.put(
                    "${" + frameServiceEntity.getServiceName() + "_HOME}",
        ...
        globalVariables.put(
                ...
                Constants.INSTALL_PATH
                        ...
                        + PackageUtils.getServiceDcPackageName(
                                clusterInfo.getClusterFrame(), "HDFS"));
    ...
    public Result runningClusterList() {
        List<ClusterInfoEntity> list =
                this.lambdaQuery()
                        .eq(ClusterInfoEntity::getClusterState, ClusterState.RUNNING)
                        .list();
        return Result.success(list);
    }
    ...
    public List<ClusterInfoEntity> getClusterByFrameCode(String frameCode) {
        return this.lambdaQuery().eq(ClusterInfoEntity::getClusterFrame, frameCode).list();
    }
