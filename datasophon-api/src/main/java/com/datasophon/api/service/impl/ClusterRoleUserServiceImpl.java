...
public class ClusterRoleUserServiceImpl
        extends ServiceImpl<ClusterRoleUserMapper, ClusterRoleUserEntity>
        implements ClusterRoleUserService {

    @Autowired private ClusterRoleUserMapper clusterRoleUserMapper;

    @Override
    public boolean isClusterManager(Integer userId, String clusterId) {
        List<ClusterRoleUserEntity> list =
                this.lambdaQuery()
                        .eq(ClusterRoleUserEntity::getUserId, userId)
    ...
    public Result saveClusterManager(Integer clusterId, String userIds) {
        this.lambdaUpdate().eq(ClusterRoleUserEntity::getClusterId, clusterId).remove();
