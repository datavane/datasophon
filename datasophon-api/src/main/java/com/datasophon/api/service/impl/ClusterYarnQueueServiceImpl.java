...
public class ClusterYarnQueueServiceImpl
        extends ServiceImpl<ClusterYarnQueueMapper, ClusterYarnQueue>
    ...
    public Result listByPage(Integer clusterId, Integer page, Integer pageSize) {
        ...
        List<ClusterYarnQueue> list =
                this.lambdaQuery()
                        .eq(ClusterYarnQueue::getClusterId, clusterId)
                        .orderByDesc(ClusterYarnQueue::getCreateTime)
        ...
        for (ClusterYarnQueue clusterYarnQueue : list) {
            String minResources =
                    clusterYarnQueue.getMinCore() + "Core," + clusterYarnQueue.getMinMem() + "GB";
            String maxResources =
                    clusterYarnQueue.getMaxCore() + "Core," + clusterYarnQueue.getMaxMem() + "GB";
            clusterYarnQueue.setMinResources(minResources);
            clusterYarnQueue.setMaxResources(maxResources);
        }

        Integer count = this.lambdaQuery().eq(ClusterYarnQueue::getClusterId, clusterId).count();
    ...
    public Result refreshQueues(Integer clusterId) throws Exception {
        List<ClusterYarnQueue> list =
                this.lambdaQuery().eq(ClusterYarnQueue::getClusterId, clusterId).list();
        // 查询resourcemanager节点
        ...
        for (ClusterYarnQueue clusterYarnQueue : list) {
            ...
            clusterYarnQueue.setMinResources(
                    minMem + "mb," + clusterYarnQueue.getMinCore() + "vcores");
            clusterYarnQueue.setMaxResources(
                    maxMem + "mb," + clusterYarnQueue.getMaxCore() + "vcores");
        ...
        ActorSelection execCmdActor =
                ActorUtils.actorSystem.actorSelection(
                        "akka.tcp://datasophon@" + hostname + ":2552/user/worker/executeCmdActor");
