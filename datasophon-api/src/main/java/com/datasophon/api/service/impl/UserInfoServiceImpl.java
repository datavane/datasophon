...
public class UserInfoServiceImpl extends ServiceImpl<UserInfoMapper, UserInfoEntity>
    ...
    public Result createUser(UserInfoEntity userInfo) {
        ...
        // 用户名判重
        Integer count =
                this.lambdaQuery().eq(UserInfoEntity::getUsername, userInfo.getUsername()).count();
    ...
    public Result getUserListByPage(String username, Integer page, Integer pageSize) {
        ...
        LambdaQueryChainWrapper<UserInfoEntity> wrapper =
                this.lambdaQuery()
                        .like(
        ...
        List<UserInfoEntity> list = wrapper.last("limit " + offset + "," + pageSize).list();
        Integer total = wrapper.count();
