...
public class ClusterServiceInstanceServiceImpl
    ...
    @Autowired private ClusterServiceInstanceMapper serviceInstanceMapper;

    @Autowired private ClusterServiceRoleInstanceService roleInstanceService;

    @Autowired private ClusterServiceDashboardService dashboardService;
    ...
    public String getServiceConfigByClusterIdAndServiceName(Integer clusterId, String serviceName) {
        return serviceInstanceMapper.getServiceConfigByClusterIdAndServiceName(
    ...
    public Result listAll(Integer clusterId) {
        ...
        List<ClusterServiceInstanceEntity> list =
                this.lambdaQuery()
                        .eq(ClusterServiceInstanceEntity::getClusterId, clusterId)
        ...
        for (ClusterServiceInstanceEntity serviceInstance : list) {
            ...
            ClusterServiceDashboard dashboard =
                    dashboardService
                            .lambdaQuery()
                            .eq(
                                    ClusterServiceDashboard::getServiceName,
            ...
            Integer alertNum =
                    alertHistoryService
                            .lambdaQuery()
                            .eq(ClusterAlertHistory::getServiceInstanceId, serviceInstance.getId())
            ...
            if (Objects.nonNull(obsoleteRoleList)
                    && obsoleteRoleList.size() == 0
    ...
    public Result delServiceInstance(Integer serviceInstanceId) {
        ...
        List<Integer> roleGroupIds =
                roleGroups.stream().map(e -> e.getId()).collect(Collectors.toList());
        ...
        roleGroupConfigService.removeByIds(
                roleGroupConfigList.stream().map(e -> e.getId()).collect(Collectors.toList()));
    ...
    public List<ClusterServiceInstanceEntity> listRunningServiceInstance(Integer clusterId) {
        return this.lambdaQuery()
                .eq(ClusterServiceInstanceEntity::getClusterId, clusterId)
                .eq(ClusterServiceInstanceEntity::getServiceState, ServiceState.RUNNING)
                .list();
    }
