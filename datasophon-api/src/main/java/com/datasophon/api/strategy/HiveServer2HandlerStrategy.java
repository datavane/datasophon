...
public class HiveServer2HandlerStrategy extends ServiceHandlerAbstract
    ...
    public void handler(Integer clusterId, List<String> hosts) {
        ...
        if (hosts.size() > 1) {
            ...
            ProcessUtils.generateClusterVariable(
                    globalVariables, clusterId, "${masterHiveServer2}", hosts.get(0));
        }
    }
    ...
    public void handlerConfig(Integer clusterId, List<ServiceConfig> list) {
        ...
        for (ServiceConfig config : list) {
            if ("enableKerberos".equals(config.getName())) {
                enableKerberos =
                        isEnableKerberos(
                                clusterId, globalVariables, enableKerberos, config, "HIVE");
            }
        }
        String key =
                clusterInfo.getClusterFrame() + Constants.UNDERLINE + "HIVE" + Constants.CONFIG;
