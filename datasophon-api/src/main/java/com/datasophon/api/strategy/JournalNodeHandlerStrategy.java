    ...
    public void handler(Integer clusterId, List<String> hosts) {
        ...
        if (hosts.size() >= 3) {
            ProcessUtils.generateClusterVariable(
                    globalVariables, clusterId, "${journalNode1}", hosts.get(0));
            ProcessUtils.generateClusterVariable(
                    globalVariables, clusterId, "${journalNode2}", hosts.get(1));
            ProcessUtils.generateClusterVariable(
                    globalVariables, clusterId, "${journalNode3}", hosts.get(2));
        }
    }
