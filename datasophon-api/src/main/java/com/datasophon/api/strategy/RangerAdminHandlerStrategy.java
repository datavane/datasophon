...
public class RangerAdminHandlerStrategy extends ServiceHandlerAbstract
    ...
    public void handler(Integer clusterId, List<String> hosts) {
        ...
        if (hosts.size() == 1) {
            ...
            ProcessUtils.generateClusterVariable(
                    globalVariables, clusterId, "${rangerAdminUrl}", rangerAdminUrl);
        }
    }
    ...
    public void handlerConfig(Integer clusterId, List<ServiceConfig> list) {
        ...
        for (ServiceConfig config : list) {
            if ("enableHDFSPlugin".equals(config.getName())
                    && ((Boolean) config.getValue()).booleanValue()) {
                logger.info("enableHdfsPlugin");
                ProcessUtils.generateClusterVariable(
                        globalVariables, clusterId, "${enableHDFSPlugin}", "true");
                enableRangerPlugin(clusterId, "HDFS", "NameNode");
            }
            if ("enableHIVEPlugin".equals(config.getName())
                    && ((Boolean) config.getValue()).booleanValue()) {
                logger.info("enableHivePlugin");
                ProcessUtils.generateClusterVariable(
                        globalVariables, clusterId, "${enableHIVEPlugin}", "true");
                enableRangerPlugin(clusterId, "HIVE", "HiveServer2");
            }
            if ("enableHBASEPlugin".equals(config.getName())
                    && ((Boolean) config.getValue()).booleanValue()) {
                logger.info("enableHbasePlugin");
                ProcessUtils.generateClusterVariable(
                        globalVariables, clusterId, "${enableHBASEPlugin}", "true");
                enableRangerPlugin(clusterId, "HBASE", "HbaseMaster");
            }
            if (config.getName().contains("Plugin") && !(Boolean) config.getValue()) {
                String configName = config.getName();
                ProcessUtils.generateClusterVariable(
                        globalVariables, clusterId, "${" + configName + "}", "false");
            }
            if ("enableKerberos".equals(config.getName())) {
                enableKerberos =
                        isEnableKerberos(
                                clusterId, globalVariables, enableKerberos, config, "RANGER");
            }
        }
        String key =
                clusterInfo.getClusterFrame() + Constants.UNDERLINE + "RANGER" + Constants.CONFIG;
    ...
    private void enableRangerPlugin(Integer clusterId, String serviceName, String serviceRoleName) {
        ...
        if (Objects.nonNull(roleList) && roleList.size() > 0) {
            ...
            List<ServiceConfig> serviceConfigs =
                    JSONObject.parseArray(config.getConfigJson(), ServiceConfig.class);
            ...
            serviceInstallService.saveServiceConfig(
                    clusterId, serviceInstance.getServiceName(), serviceConfigs, roleGroupId);
        }
    }
}
