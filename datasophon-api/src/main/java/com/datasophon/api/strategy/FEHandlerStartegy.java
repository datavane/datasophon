...
public class FEHandlerStartegy implements ServiceRoleStrategy {
    ...
    public void handler(Integer clusterId, List<String> hosts) {
        ...
        if (hosts.size() >= 1) {
            ProcessUtils.generateClusterVariable(
                    globalVariables, clusterId, "${feMaster}", hosts.get(0));
        }
    }
    ...
    public void handlerServiceRoleCheck(
        ...
        Map<String, String> globalVariables =
                GlobalVariables.get(roleInstanceEntity.getClusterId());
        String feMaster = globalVariables.get("${feMaster}");
    ...
    private void resolveProcInfoAlert(
            String serviceRoleName,
            List<ProcInfo> frontends,
            Map<String, ClusterServiceRoleInstanceEntity> map) {
        for (ProcInfo frontend : frontends) {
            ClusterServiceRoleInstanceEntity roleInstanceEntity =
                    map.get(frontend.getHostName() + serviceRoleName);
            if (!frontend.getAlive()) {
                ...
                ProcessUtils.saveAlert(
                        roleInstanceEntity, alertTargetName, AlertLevel.WARN, alertAdvice);
            } else {
