...
public class InstallServiceHandler {
    ...
    public ExecResult install(
            String packageName, String decompressPackageName, String packageMd5, RunAs runAs) {
        ExecResult execResult = new ExecResult();
        try {
            String destDir =
                    Constants.INSTALL_PATH + Constants.SLASH + "DDP/packages" + Constants.SLASH;
    ...
    private void downloadPkg(String packageName, String packagePath) {
        ...
        String downloadUrl =
                "http://"
                        ...
                        + ":"
                        + masterPort
                        + "/ddh/service/install/downloadPackage?packageName="
                        + packageName;
    ...
    private boolean decompressPkg(
            String packageName, String decompressPackageName, RunAs runAs, String packagePath) {
        if (!FileUtil.exist(Constants.INSTALL_PATH + Constants.SLASH + decompressPackageName)) {
            Boolean decompressResult =
                    CompressUtils.decompressTarGz(packagePath, Constants.INSTALL_PATH);
                ...
                if (decompressPackageName.contains(Constants.PROMETHEUS)) {
                    String alertPath =
                            Constants.INSTALL_PATH
                                    ...
                                    + "alert_rules";
                    ShellUtils.exceShell(
                            "sed -i \"s/clusterIdValue/"
                                    + PropertyUtils.getString("clusterId")
                                    + "/g\" `grep clusterIdValue -rl "
                                    + alertPath
                                    + "`");
                }
