...
public class NameNodeHandlerStrategy implements ServiceRoleStrategy {
    ...
    public ExecResult handler(ServiceRoleOperateCommand command ) {
        ...
        if(command.getCommandType().equals(CommandType.INSTALL_SERVICE)){
            ...
            ArrayList<String> commands = new ArrayList<>();
            commands.add("sh");
            commands.add("/opt/datasophon/hadoop-3.3.3/ranger-hdfs-plugin/enable-hdfs-plugin.sh");
            if(!FileUtil.exist("/opt/datasophon/hadoop-3.3.3/ranger-hdfs-plugin/success.id")){
                ExecResult execResult = ShellUtils.execWithStatus("/opt/datasophon/hadoop-3.3.3/ranger-hdfs-plugin", commands, 30L);
                if(execResult.getExecResult()){
                    logger.info("enable ranger hdfs plugin success");
                    //写入ranger plugin集成成功标识
                    FileUtil.writeUtf8String("success","/opt/datasophon/hadoop-3.3.3/ranger-hdfs-plugin/success.id");
                    startResult = serviceHandler.start(command.getStartRunner(),command.getStatusRunner(),command.getDecompressPackageName(),command.getRunAs());
                }else{
