...
public class HbaseHandlerStrategy implements ServiceRoleStrategy{
    ...
    public ExecResult handler(ServiceRoleOperateCommand command) {
        ...
        if(command.getEnableRangerPlugin()){
            ...
            if(!FileUtil.exist("/opt/datasophon/hbase-2.2.7/ranger-hbase-plugin/success.id")) {
                ExecResult execResult = ShellUtils.execWithStatus("/opt/datasophon/hbase-2.2.7/ranger-hbase-plugin", commands, 30L);
                if (execResult.getExecResult()) {
                    logger.info("enable ranger hbase plugin success");
                    FileUtil.writeUtf8String("success","/opt/datasophon/hbase-2.2.7/ranger-hbase-plugin/success.id");
