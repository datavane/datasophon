        ...
        if (command.getEnableKerberos()) {
            ...
            if (!FileUtil.exist(hadoopConfDir + "ssl-server.xml")) {
                ShellUtils.exceShell(
                        "cp " + hadoopConfDir + "ssl-server.xml.example ssl-server.xml");
            }
            if (!FileUtil.exist(hadoopConfDir + "ssl-client.xml")) {
                ShellUtils.exceShell(
                        "cp " + hadoopConfDir + "ssl-client.xml.example ssl-client.xml");
            }
            ...
            if (!FileUtil.exist("/etc/security/keytab/keystore")) {
                ...
                ExecResult execResult =
                        ShellUtils.execWithStatus(Constants.WORKER_SCRIPT_PATH, commands, 30L);
