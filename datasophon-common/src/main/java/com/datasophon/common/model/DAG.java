...
/**
 * analysis of DAG Node: node NodeInfo：node description information EdgeInfo: edge description
 * information
 */
public class DAG<Node, NodeInfo, EdgeInfo> {
    ...
    /**
     * reversed edge set，key is node of destination, value is Map with key for origin node and value
     * for edge
     */
    ...
    private void addEdge(
            Node fromNode, Node toNode, EdgeInfo edge, Map<Node, Map<Node, EdgeInfo>> edges) {
     ...
     *
     * <p>Directed acyclic graph (DAG) has topological ordering Breadth First Search： 1、Traversal of
     * all the vertices in the graph, the degree of entry is 0 vertex into the queue 2、Poll a vertex
     * in the queue to update its adjacency (minus 1) and queue the adjacency if it is 0 after minus
     * 1 3、Do step 2 until the queue is empty If you cannot traverse all the nodes, it means that
     * the current graph is not a directed acyclic graph. There is no topological sort.
     *
     * @return key Returns the state if success (acyclic) is true, failure (acyclic) is looped, and
     *     value (possibly one of the topological sort results)
     */
    private Map.Entry<Boolean, List<Node>> topologicalSortImpl() {
        ...
        /**
         * After scanning, there is no node with 0 degree of entry, indicating that there is a ring,
         * and return directly
         */
        ...
        // The topology algorithm is used to delete nodes with 0 degree of entry and its associated
        // edges
